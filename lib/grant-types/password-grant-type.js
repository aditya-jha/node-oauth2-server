 ...
 */
 ...
 PasswordGrantType.prototype.handle = function(request, client) {
   ...
   return Promise.bind(this)
     ...
     .then(function(user) {
       user_result = user; 
       return this.deletePreviousTokens(user, client)
     })
     .then(function() {
       return this.saveToken(user_result, client, scope)
     });
 };
...
PasswordGrantType.prototype.getUser = function(request) {
  ...
  return Promise.try(this.model.getUser, [request.body.username, request.body.password])
    .then(function(user) {
      console.log("PasswordGrantType User :" + JSON.stringify(user));
      if (!user) {
...
PasswordGrantType.prototype.deletePreviousTokens = function(user, client) {
  return this.model.deletePreviousTokens(user, client);
}
